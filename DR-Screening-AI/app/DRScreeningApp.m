        ...
        function run(app)
            ...
            try
                ...
                summary = {
                    ...
                    '========================================================================================', ...
                    sprintf('Timestamp        : %s', char(res.timestamp)), ...
                    sprintf('Quality Status   : %s (Quality Score: %.1f%%)', char(res.qualityStatus), res.qualityScore * 100), ...
                    sprintf('Predicted Class  : Stage %d - %s', res.predictedClass, char(res.predictedLabel)), ...
                ...
                if isfield(res, 'classProbabilities') && ~isempty(res.classProbabilities)
                    ...
                    for k = 1:min(length(cfg.classNames), length(res.classProbabilities))
                        summary{end+1} = sprintf('  [%d] %-30s : %5.1f%%', k-1, cfg.classNames(k), res.classProbabilities(k) * 100);
                    end
                end
                
                summary{end+1} = '----------------------------------------------------------------------------------------';
                summary{end+1} = sprintf('Clinical Action  : %s', char(res.recommendation));
                summary{end+1} = '----------------------------------------------------------------------------------------';
                ...
                app.StatusLabel.Text = sprintf('Screening completed. Result: %s (Confidence: %.1f%%)', char(res.predictedLabel), res.confidence * 100);
