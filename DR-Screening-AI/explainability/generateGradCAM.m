...
if nargin < 3 || isempty(classIndex), classIndex = 1; end

classIndex = round(max(1, classIndex));

cam = [];
if ~isempty(net)
    try
        cam = gradCAM(net, single(image), classIndex, 'ReductionLayer', layerName);
        cam = safeMat2Gray(cam);
    catch
        cam = [];
    end
end
...
if isempty(cam)
    % Fallback feature activation saliency map based on local spatial gradient energy
...
% Generate colormap overlay natively in MATLAB
cmap = jet(256);
...
elseif size(dispImgDouble, 3) > 3
    dispImgDouble = dispImgDouble(:,:,1:3);
end
